...
//  Project5
//
//  Created by Ryordan Panter on 25/6/20.
...
class ViewController: UITableViewController {
    ...
    @objc func promptForAnswer() {
        let ac = UIAlertController(title: "Enter anwer", message: nil, preferredStyle: .alert)
        ac.addTextField()
...
//                print(tempWord)
    ...
    func isReal(word: String) -> Bool {
        ...
        if misspelledRange.location == NSNotFound && word.count >= 3 && word != title?.lowercased(){
            return true
        } else {
            return false
        }
